    ...
    for (size_t i = 0; i < m.size(); i++) {
     ...
     if (_c1 != c1 || com.c2.size() != m.size()) {
        ...
        for (size_t i = 0; i < m.size(); i++) {
             _c2[i] = f * com.c2[i];
            if (_c2[i] != c2[i]) {
                cout << "ERROR: Commit opening failed test for c2 (postition " << i << ")" << endl;
                result = false;
            }
        }
...
void commit_doit(commit_t& com, cipher_t& c, commitkey_t& key, vector<params::poly_q> r) {
    params::poly_q _m;
...
static void test2() {
    ...
    vector<params::poly_q> _m = { 0 }, m = { nfl::uniform(), nfl::uniform() };
