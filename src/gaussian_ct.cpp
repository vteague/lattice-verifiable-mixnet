...
static inline void box_muller() {
	...
	nfl::fastrandombytes(r, BOX_MULLER_BYTES);
...
static inline __m128d exp_constant(const __m128d x) {
	...
	/* evaluate 2^a */
	vsum = _mm_add_sd(_mm_mul_sd(_mm_castsi128_pd(_mm_cvtsi64x_si128(EXP_COFF
									[0])), vx2),
			_mm_castsi128_pd(_mm_cvtsi64x_si128(EXP_COFF[1])));
	vsum = _mm_add_sd(_mm_mul_sd(vsum, vx2),
	...
	vsum = _mm_add_sd(_mm_mul_sd(vsum, vx2),
			_mm_castsi128_pd(_mm_cvtsi64x_si128(EXP_COFF[10])));
...
static inline int64_t comp(const unsigned char *r, const uint64_t res) {
	...
	return (((1LL << 63) ^ ((res - DOUBLE_ONE) | (DOUBLE_ONE -
									res))) | ((r_mantissa -
							res_mantissa) & (r_exponent -
							(1LL << res_exponent)))) >> 63;
}
...
int64_t discrete_gaussian(const double center) {
	...
	_mm_storel_epi64((__m128i *) (&drc),
			_mm_castpd_si128(_mm_mul_sd(exp_constant(_mm_mul_sd(vc, vc)),
							V_DISCRETE_NORMALISATION)));

	nfl::fastrandombytes(r, COMP_ENTRY_SIZE);
	dcmprc = -comp(r, drc);
	vcmprc = _mm_loadl_epi64((__m128i *) (&dcmprc));

	while (true) {
		nfl::fastrandombytes(r, DISCRETE_BYTES);
		...
		for (i = 0; i < 8; i++, head++) {
			...
			vyr = _mm_add_sd(_mm_round_pd(vy,
							_MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC),
					_mm_castsi128_pd(_mm_xor_si128(_mm_castpd_si128(V_M_1_NEG),
									_mm_and_si128(vb,
											_mm_xor_si128(_mm_castpd_si128
													(V_M_1_NEG),
													_mm_castpd_si128
													(V_M_1))))));
			...
			if (dcmp) {
				...
				_mm_storel_epi64((__m128i *) (&drej),
						_mm_castpd_si128(exp_constant(_mm_mul_sd(_mm_add_sd
												(vyrc, vy), _mm_sub_sd(vyrc,
														vy)))));

				if (comp(r + i * COMP_ENTRY_SIZE, drej)) {
					head++;
					return _mm_cvtsd_si64(_mm_add_sd(_mm_castsi128_pd
									(_mm_xor_si128(_mm_castpd_si128(vyr),
													_mm_and_si128(vcmprc,
															_mm_castpd_si128
															(vyr)))), vcr));
				}
			}
...
int main() {
	...
	for (t = 0; t < T; t++) {
		...
		nfl::fastrandombytes_setseed(seed);
