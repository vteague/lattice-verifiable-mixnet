...
static void piaex_hash(fmpz_t x, fmpz_t beta0, fmpz_t beta[TAU][3], fmpz_t q, commit_t& com) {
    SHA256Context sha;
...
static void poly_encode(params::poly_big& out, fmpz_mod_poly_t in0, fmpz_mod_poly_t in1, fmpz_t in[ETA], const fmpz_mod_ctx_t ctx) {
...
static void poly_from(fmpz_mod_poly_t& out, params::poly_q& in, const fmpz_mod_ctx_t ctx) {
    array<mpz_t, params::poly_q::degree> coeffs;
...
static int piaex_prover(commit_t& com, fmpz_t x, fmpz_mod_poly_t f[V], fmpz_t rf[ETA],
        fmpz_mod_poly_t h[2][V], fmpz_t rh[ETA], vector<params::poly_q> rd,
        commitkey_t& key, fmpz_mod_poly_t lag[TAU + 1],
        flint_rand_t prng, const fmpz_mod_ctx_t ctx) {
...
static int piaex_verifier(commit_t& com, fmpz_mod_poly_t f[V], fmpz_t rf[ETA],
        vector<params::poly_q> rd, commitkey_t& key, fmpz_mod_poly_t lag[TAU + 1],
        flint_rand_t prng, const fmpz_mod_ctx_t ctx) {
    array<mpz_t, params::poly_q::degree> coeffs;
...
static void test(flint_rand_t rand) {
    ...
    TEST_ONCE("AEX proof is consistent") {
        ...
        TEST_ASSERT(piaex_verifier(com, f, rf, rd, key, lag, rand, ctx) == 1, end);
	} TEST_END;
